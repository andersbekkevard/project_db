...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Registrer oppmote ved hjelp av SQL-foerst-logikk."
    )
...
def run_attendance(args: argparse.Namespace):
    connection = db.connect()
    try:
        with db.transaction(connection):
            connection.execute("DROP TABLE IF EXISTS temp.uc3_input")
            connection.execute(
                """
                CREATE TEMP TABLE uc3_input (
                    epost TEXT NOT NULL,
            ...
            connection.execute(
                """
                INSERT INTO uc3_input (epost, aktivitet, tidspunkt, referansetid)
                VALUES (?, ?, ?, ?)
                """,
                (args.epost, args.aktivitet, args.tidspunkt, args.referansetid),
            )
            connection.executescript(db.load_sql("30_uc3_registrer_oppmote.sql"))
            result = connection.execute(
                "SELECT * FROM uc3_result LIMIT 1"
            ).fetchone()
            if result is None:
                raise RuntimeError("SQL-skriptet returnerte ingen oppmotestatus.")
            return result
    finally:
        connection.close()
...
def main() -> int:
    args = parse_args()
    try:
        result = run_attendance(args)
    except Exception as exc:
        db.write_output(f"Registrering av oppmote feilet: {exc}")
        return 1
