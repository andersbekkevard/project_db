...
CREATE TABLE IF NOT EXISTS gruppetime (
    ...
    senter_id INTEGER NOT NULL,
    sal_nr INTEGER NOT NULL,
    starttidspunkt TEXT NOT NULL,
    sluttidspunkt TEXT NOT NULL,
    ...
    CHECK (datetime(starttidspunkt) IS NOT NULL),
    CHECK (datetime(sluttidspunkt) IS NOT NULL),
    CHECK (datetime(sluttidspunkt) > datetime(starttidspunkt))
);
...
CREATE TABLE IF NOT EXISTS deltar_på_time (
    ...
    CHECK (oppmøtt_tidspunkt IS NULL OR datetime(oppmøtt_tidspunkt) IS NOT NULL),
    ...
    CHECK (
        oppmøtt_tidspunkt IS NULL
        OR datetime(oppmøtt_tidspunkt) >= datetime(påmeldt_tidspunkt)
    ),
    CHECK (
